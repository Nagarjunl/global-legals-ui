...
import Logo from '../../assets/GlobalLegals.png';
...
const AdminSidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-9999 flex shadow-2  h-screen w-60 flex-col overflow-y-hidden bg-white duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between shadow-2 gap-2 px-6 py-2">
        <NavLink to="/">
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
              ...
              <li>
                <NavLink
                  to="addFaq"
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-normal duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    pathname.includes('addFaq') && 'bg-gray-300'}`}
                    onClick={() => setSidebarOpen(!sidebarOpen)}
                >
                  <svg
                    className="fill-current"
